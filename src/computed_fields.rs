...
impl ComputedFields {
    ...
    fn add_megapixels(metadata: &mut HashMap<String, String>) {
        if let (Some(width), Some(height)) =
            (metadata.get("ImageWidth"), metadata.get("ImageHeight"))
        {
    ...
    fn add_fov(metadata: &mut HashMap<String, String>) {
        if let (Some(focal_length), Some(sensor_width)) =
            (metadata.get("FocalLength"), metadata.get("SensorWidth"))
        {
            if let (Ok(fl), Ok(sw)) = (
                Self::parse_focal_length(focal_length),
    ...
    fn add_hyperfocal_distance(metadata: &mut HashMap<String, String>) {
        if let (Some(focal_length), Some(aperture)) =
            (metadata.get("FocalLength"), metadata.get("FNumber"))
        {
            if let (Ok(fl), Ok(f)) = (
                Self::parse_focal_length(focal_length),
                aperture.parse::<f64>(),
                ...
                if fl > 0.0 && f > 0.0 {
                    ...
                    metadata.insert(
                        "HyperfocalDistance".to_string(),
                        format!("{:.1} m", hyperfocal / 1000.0),
                    );
                }
            }
        }
    ...
    fn add_composite_fields(metadata: &mut HashMap<String, String>) {
        ...
        if let Some(wb) = metadata.get("WhiteBalance") {
            ...
            metadata.insert(
                "Composite:BlueBalance".to_string(),
                blue_balance.to_string(),
            );
        }
        ...
        if let (Some(lens_make), Some(lens_model)) =
            (metadata.get("LensMake"), metadata.get("LensModel"))
        {
    ...
    fn insert_subsec_datetime(
        ...
        let offset =
            Self::first_nonempty(metadata, offset_keys).filter(|tz| Self::looks_like_offset(tz));
        metadata.insert(
            dest.to_string(),
