...
use crate::parsers::{JpegParser, RawParser, HeifParser, VideoParser};
    ...
    fn process_files_parallel(&self, file_paths: Vec<String>) -> Result<HashMap<String, PyObject>, ExifError> {
        ...
        Python::with_gil(|py| {
            ...
            for result in results {
                let file_key = format!("file_{}", result.file_path.replace("/", "_").replace("\\", "_"));
...
#[pyfunction]
pub fn process_files_parallel(file_paths: Vec<String>, max_workers: Option<usize>) -> PyResult<PyObject> {
    Python::with_gil(|py| {
...
#[pyfunction]
pub fn process_directory_parallel(directory: String, extensions: Option<Vec<String>>, max_files: Option<usize>, max_workers: Option<usize>) -> PyResult<PyObject> {
    Python::with_gil(|_py| {
        let reader = MultiprocessingExifReader::new(max_workers);
