...
try:
    ...
    click.echo(
        "Error: fast_exif_reader module not found. Please install the package first.",
        err=True,
    )
    sys.exit(1)
...
class ExifFormatter:
    ...
    def format_very_short(self, metadata: dict[str, str], filename: str) -> str:
        ...
        for _tag, value in sorted(metadata.items()):
            lines.append(value)
    ...
    for file_path in files:
        if file_path.is_file():
            ...
            if recursive:
                ...
                click.echo(
                    f"Warning: {file_path} is a directory. Use -r for recursive processing.",
                    err=True,
                )
        elif not quiet:
            click.echo(
