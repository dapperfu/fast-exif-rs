#!/usr/bin/env python3
"""
Benchmark script comparing fast-exif-rs EXIF writing performance against exiftool.
Tests various scenarios with temporary images to ensure fair comparison.
"""
...
try:
    ...
    from fast_exif_reader import BatchExifWriter, write_exif_batch_parallel
except ImportError:
...
class ExifBenchmark:
    ...
    def create_test_images(self, count: int = 10) -> List[str]:
        """Create test JPEG images with basic EXIF data"""
        test_images = []
        
        for i in range(count):
            # Create a simple test image using ImageMagick or PIL
            try:
                ...
                exif_dict = {
                    ...
                    33434: (1, 100),    # ExposureTime
                    33437: (28, 10),    # FNumber
                    34855: 100,         # ISO
                    37386: (50, 1),     # FocalLength
                }
                
                exif_bytes = ExifTags.dump(exif_dict)
                img.save(os.path.join(self.temp_dir, f"test_image_{i:03d}.jpg"), exif=exif_bytes)
                test_images.append(os.path.join(self.temp_dir, f"test_image_{i:03d}.jpg"))
                
            except ImportError:
                # Fallback: create minimal JPEG files using ImageMagick
                try:
                    cmd = [
                        "convert", "-size", "1920x1080", "xc:red",
                        "-set", "exif:Make=Test Camera",
                        "-set", "exif:Model=Test Model", 
                        "-set", "exif:DateTime=2024:01:01 12:00:00",
                        os.path.join(self.temp_dir, f"test_image_{i:03d}.jpg")
                    ]
                    subprocess.run(cmd, check=True, capture_output=True)
                    test_images.append(os.path.join(self.temp_dir, f"test_image_{i:03d}.jpg"))
                except (subprocess.CalledProcessError, FileNotFoundError):
                    print("Warning: Could not create test images. Using existing images from test_files/")
                    # Use existing test images if available
                    test_files_dir = Path(__file__).parent / "test_files" / "essential"
                    if test_files_dir.exists():
                        existing_images = list(test_files_dir.glob("*.jpg"))[:count]
                        for img_path in existing_images:
                            dest_path = os.path.join(self.temp_dir, img_path.name)
                            shutil.copy2(img_path, dest_path)
                            test_images.append(dest_path)
                    else:
                        raise RuntimeError("No test images available and cannot create them")
    ...
    def benchmark_fast_exif_rs_single(self, input_path: str, output_path: str, metadata: Dict[str, str]) -> Tuple[float, bool]:
        ...
        try:
            ...
            writer = fast_exif_reader.ExifWriter()
            success = writer.write_exif(input_path, output_path, metadata)
    ...
    def benchmark_fast_exif_rs_batch(self, operations: List[Dict]) -> Tuple[float, Dict]:
        ...
        try:
            ...
            # Use batch writer
            results = write_exif_batch_parallel(batch_ops, max_workers=None)
    ...
    def run_comprehensive_benchmark(self):
        """Run comprehensive benchmark comparing both tools"""
        print("🚀 Starting Comprehensive EXIF Writing Benchmark")
        print("=" * 60)
        ...
        try:
            # Test scenarios
            test_scenarios = [
                {"name": "Small Batch (5 files)", "count": 5},
                {"name": "Medium Batch (20 files)", "count": 20},
                {"name": "Large Batch (50 files)", "count": 50},
                {"name": "Very Large Batch (100 files)", "count": 100},
            ]
            ...
            for scenario in test_scenarios:
                ...
                # Create test images
                test_images = self.create_test_images(scenario['count'])
                ...
                # Single file tests
                for i, op in enumerate(operations[:5]):  # Test first 5 files individually
                ...
                # Batch test
                batch_time, batch_results = self.benchmark_fast_exif_rs_batch(operations)
                ...
                # Single file tests
                for i, op in enumerate(exif_operations[:5]):  # Test first 5 files individually
    ...
    def save_results(self, filename: str = "exif_writing_benchmark_results.json"):
        ...
        self.results["metadata"] = {
            ...
            "test_environment": "Temporary Images"
        }
...
def main():
    ...
    print("Using temporary test images for fair comparison")
    print()
