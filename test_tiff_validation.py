...
def test_tiff_validation():
    ...
    # Check byte order
    print("\nByte order check:")
    print(f"Invalid: {invalid_tiff[:2]} -> {'II' if invalid_tiff[:2] == b'II' else 'MM' if invalid_tiff[:2] == b'MM' else 'Unknown'}")
    print(f"Valid: {valid_tiff[:2]} -> {'II' if valid_tiff[:2] == b'II' else 'MM' if valid_tiff[:2] == b'MM' else 'Unknown'}")
