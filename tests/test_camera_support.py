...
def create_test_data(test_case):
    ...
    if test_case['format'] == 'JPEG':
        ...
        return b'\xff\xd8\xff\xe1\x00\x16Exif\x00\x00II*\x00\x08\x00\x00\x00\x01\x00\x0f\x01\x02\x00\x05\x00\x00\x00\x1a\x00\x00\x00\x00\x00\x00\x00' + test_case['make'].encode() + b'\x00' + test_case['model'].encode() + b'\x00'
    elif test_case['format'] == 'NEF':
        # Minimal TIFF header for Nikon
        return b'II*\x00\x08\x00\x00\x00\x01\x00\x0f\x01\x02\x00\x05\x00\x00\x00\x1a\x00\x00\x00\x00\x00\x00\x00' + test_case['make'].encode() + b'\x00' + test_case['model'].encode() + b'\x00'
    elif test_case['format'] == 'HEIF':
        # Minimal HEIF header
        return b'\x00\x00\x00\x20ftypheic\x00\x00\x00\x00heic\x00\x00\x00\x00' + test_case['make'].encode() + b'\x00' + test_case['model'].encode() + b'\x00'
    else:
