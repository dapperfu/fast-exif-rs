#!/usr/bin/env python3
"""
Corrected Maximum Performance Benchmark for fast-exif-rs
Tests ALL available optimizations with correct API usage
"""
...
def benchmark_multiprocessing_reader(files: List[str]) -> Dict[str, Any]:
    """Benchmark the MultiprocessingExifReader with correct API"""
...
def benchmark_batch_processing(files: List[str]) -> Dict[str, Any]:
    ...
    try:
        ...
        batch_sizes = [1, 5, 10, min(20, len(files))]
        batch_results = {}
...
def benchmark_directory_processing(directory: str) -> Dict[str, Any]:
    """Benchmark directory processing"""
    print("📁 Benchmarking Directory Processing...")
    
    try:
        import fast_exif_reader
        
        start_time = time.time()
        results = fast_exif_reader.read_directory(directory)
        end_time = time.time()
        
        total_time = end_time - start_time
        
        if isinstance(results, dict):
            file_count = len([k for k in results.keys() if k != 'stats'])
            success_count = len([v for v in results.values() if hasattr(v, 'success') and v.success])
            
            return {
                "reader_type": "Directory Processing",
                "total_files": file_count,
                "successful_files": success_count,
                "success_rate": success_count / file_count * 100 if file_count > 0 else 0,
                "total_time": total_time,
            ...
            return {"error": "Invalid results format", "reader_type": "Directory Processing"}
        
    except Exception as e:
        return {"error": f"Failed to benchmark directory processing: {e}", "reader_type": "Directory Processing"}
...
def main():
    ...
    test_files = find_test_files(test_directory, max_files=25)  # Reasonable set for testing
    ...
    directory_result = benchmark_directory_processing(test_directory)
