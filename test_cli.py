...
def test_cli():
    ...
    print("\n1. Testing help:")
    try:
        result = subprocess.run([str(cli_path), cli_module, "--help"], 
    ...
    print("\n2. Testing version:")
    try:
        result = subprocess.run([str(cli_path), cli_module, "--version"], 
                              capture_output=True, text=True)
        if result.returncode == 0:
    ...
    try:
        result = subprocess.run([str(cli_path), cli_module], 
                              capture_output=True, text=True)
    ...
    try:
        result = subprocess.run([str(cli_path), cli_module, "nonexistent.jpg"], 
                              capture_output=True, text=True)
