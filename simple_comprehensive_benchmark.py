...
- Different file counts (10, 20, 50, 100)
...
class BenchmarkResults:
    ...
    def add_result(self, method: str, file_count: int, thread_count: int, 
                   times: List[float], success_count: int, error_count: int) -> None:
    ...
    def print_summary(self) -> None:
        ...
        for file_count in file_counts:
            ...
            print(f"{'Method':<25} {'Threads':<8} {'Total(s)':<8} {'Files/s':<8} {'MB/s':<8} {'Success%':<8}")
            print("-" * 100)
...
def benchmark_python_single_threaded(files: List[str]) -> Tuple[List[float], int, int]:
...
def benchmark_python_multiprocessing(files: List[str], max_workers: int) -> Tuple[List[float], int, int]:
...
def benchmark_rust_single_threaded(files: List[str]) -> Tuple[List[float], int, int]:
...
def benchmark_rust_multiprocessing(files: List[str], max_workers: int) -> Tuple[List[float], int, int]:
            ...
            for method_name, benchmark_func in methods:
                ...
                for run in range(num_runs):
                    ...
                    if "Single-threaded" in method_name:
                        times, success, errors = benchmark_func(test_files)
                    else:
                        times, success, errors = benchmark_func(test_files, thread_count)
                ...
                if all_times:
                    results.add_result(method_name, file_count, thread_count, 
                                     all_times, all_success, all_errors)
...
def main():
    ...
    parser.add_argument("--file-counts", nargs="+", type=int, default=[10, 20, 50],
                       help="File counts to test (default: 10 20 50)")
